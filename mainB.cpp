...
bool readTest(){//读取测试集
	...
	//更新测试日志
	ofstream out;
	out.open("test_logB.txt",ios::out|ios::ate|ios::app);
	time_t t;
	time(&t);
...
void ReadRGB(IMAGE* Img){//读取RGB值 导入图像数组
	...
	for(int i=0;i<(ImageLengthOriginal*ImageLengthOriginal);i++){
		...
		//datapiece[1][i/ImageLengthOriginal+1][i%ImageLengthOriginal+1]=(UCHAR)(color>>8);//获取绿色 Green值  
		datapiece[i/ImageLengthOriginal+1][i%ImageLengthOriginal+1]=(UCHAR)(color>>16);//获取蓝色 Blue值
	}
